...
import { SESSION_COOKIE, getUserAuthByUsername, getUserByUsername, setSessionCookie, validatePassword, verifyPassword } from "@/lib/auth";
...
export async function POST(request: Request) {
  ...
  if (!validatePassword(password)) {
    return NextResponse.json({ error: "請提供至少 8 碼密碼。" }, { status: 400 });
  }
