...
> `excited` 目前確認為姿態型。後續可依實際產出結果微調分類。

### 基準狀態

| state_id | 情緒目標 | 用途 | 是否對外發布 |
|---|---|---|---|
| `default` | 中性吉祥物基線 | 作為流水線內部比對的錨定圖 | 否，僅內部使用 |

> **決策**：`default` 僅作為資產流水線內部基線，不對外發布。產品端 fallback 與對外展示狀態統一使用 `calm`（見 [Product PRD §8](./product-prd.md#8-寵物系統需求產品層)）。
...
<!-- 備註：各狀態的「表情型/姿態型」分類已確認（含 excited 為姿態型），後續可依實際產出結果微調 -->
...
> 系統取圖策略定義於 [Product PRD §8](./product-prd.md#8-寵物系統需求產品層)，此處不重複。素材生產時需確保至少 `calm` 狀態圖存在，以滿足 fallback 鏈。
